...
const _Desk = (dsk&&dsk.Desk) || Core.Desk;
...
const editsave=async(if_version)=>{//«
	...
	func(usepath, val, (ret,err_or_pos)=>{
		if (ret) {
			if (_Desk && _Desk.make_icon_if_new(usepath)) _Desk.update_folder_statuses();
			let numstr="";
			if (if_version) {
//				numstr = ;
...
fs.path_to_obj(pardir,parobj=>{
	...
	let checkok=_=>{
		rtype = rootobj.TYPE;
		if (rtype=="remote") {
			if (!fs.check_user_perm(fobj)) return "Permission denied: " + fname;
			return true;
		}
...
else if (com=="w"||com=="write")stat_render("Alt+s to save!");
