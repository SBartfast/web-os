...
const wincontext=(loc, win, dsk)=>{//«
	...
	if (win.obj._hashsum) {
		menu.add_item("Update",()=>{
			let popup = make_popup({STR: "Checking for updates...", WIN: win, NOBUTTONS:true});
			Core.do_update(`apps.${app}`, win.obj._hashsum,rv=>{
				popup.ok();
				if (rv) {
					popok(`Updated OK: ${name}`,{win: win});
					delete NS.apps[win.app];
					NS.apps[win.app]=undefined;
				}
				else if (rv===false) poperr(`Could not update to the new version: ${name}`,{win:win});
				else make_popup({STR:`The server has the same version of app: '${name}'`, WIN:win});
			});
		});
	}
	else if (!arr.length){
...
const make_popup = (arg, dsk) => {//«
	const popup_dequeue=()=>{setTimeout(function(){let args=_popup_queue.shift();make_popup(args,dsk);},250);}
