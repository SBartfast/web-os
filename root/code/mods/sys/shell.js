...
const{dev_mode,dev_env,util,fs}=globals;
...
const _get_rem_file=(path,cb,if_no_cache,if_local,if_text,if_bytes)=>{let fullpath=get_fullpath(path);if(!fullpath)return cb();fs.get_rem_file(fullpath,cb,{ASBYTES:if_bytes,TEXT:if_text,NOCACHE:if_no_cache},if_local);}
...
const readFile=(path,opts={})=>{
	opts.DSK=dsk;
	if (!path.match(/^\//)) path = (cur_dir + "/" + path).regpath();
...
Most commands exist inside of command libraries which can be imported via the 'import' command.\n
...
~$ import fs\n~$ help vim\n
`;
let nlobj = {t: "c_op", c_op: "nl", nl: true};
let EOF = {EOF: true};
termobj.EOF = EOF;
let is_eof = val=>{return (val === EOF);}
let builtins = null;
let sys_builtins;
let parser = null;
...
const toks_to_string=(arr,if_nospace)=>{let str="";for(let i=0;i<arr.length;i++){let tok=arr[i];let is_nl=false;if(tok===undefined)continue;let t=tok.t;if(tok=="")continue;if(typeof(tok)=="string" || typeof(tok)=="number")str+=tok;else if(tok.EOF)return str;else if(typeof(tok[t])=="string"){if(t=="c_op" && tok[t]=="nl"){is_nl=true;str+="\n";}else str+=tok[t];}else if(t=="sub"){let subt=tok.sub_t;let beg="$";let end="";if(subt=="com"){beg="$(";end=")";}else if(subt=="math"){beg+="$((";end="))";}str+=beg+toks_to_string(tok.sub,if_nospace)+end;}else if(t=="quote"){let quote=tok.quote;let qt=tok.quote_t;if(quote.t=="quote_string")str=str+tok.quote.quote_string;else if(quote.length !=undefined &&(qt=="'" || qt=='"' || qt=='\x60')){let qarr=tok.quote;for(let j=0;j<qarr.length;j++){let ch=qarr[j];if(typeof ch=="string"){}else if(typeof ch=="object" && ch.t=="esc")qarr[j]="\\"+ch.esc;}let qstr=qt+qarr.join("")+qt;if(tok['$'])qstr="$"+qstr;str=str+qstr;}else{log("\n\nWhat kinda quote HAR NULLLLLLL???\n\n");return null;}}else if(t=="group_ret"){let group=tok['group_ret'];if(group && group.length !=undefined)str+=group.join(" ");else{}}else{return tok;}if(!if_nospace && !is_nl && i+1<arr.length)str=str+" ";}return str;}
...
const pathtocontents=(arg,cb,if_dat,nbytes)=>{path_to_obj(arg,ret=>{if(!ret)return cb();if(ret.root.TYPE=="fs")fs.get_fs_by_path(arg,cb,{BLOB:if_dat,NBYTES:nbytes,ROOT:is_root});else if(ret.root.TYPE=="remote")fs.get_rem_file(arg,cb,{ASBYTES:true});else{cwarn("pathtocontents():Not handling root type:"+ret.root.TYPE);cb();}});}
...
const groupify=(toks,opts)=>{let is_quote;let from_math;let global_com_name;let main_com_str_arg;if(opts){is_quote=opts.QUOTE;from_math=opts.MATH;global_com_name=opts.NAME;main_com_str_arg=opts.COM;}let db=(str)=>{if(is_quote)log(str);};let cur_line=0;let make_math_group=(grp,if_ds,pos1,pos2,which)=>{if(if_ds)toks[pos1]={'t':'sub','sub_t':"math",'sub':grp,ln:cur_line};else toks[pos1]={'t':'group','group_t':"math",'math':true,'group':grp,ln:cur_line};toks.splice(pos1+1,pos2-pos1+1);};let cur_error_message=null;if(toks==null)return null;let quote_str=null;let quote_start=null;let quote_num=0;let have_comment=null;let have_prev_c_op;let heredoc_word=null;for(let i=0;i<toks.length;i++){let tok=toks[i];let tok1=toks[i+1];if(heredoc_word && tok.word==heredoc_word){heredoc_word=null;continue;}else if(!heredoc_word && tok.r_op=="<<"){let next=find_not_blank(toks,i+1);if(next&&next.t=="word"){i=next.pos;heredoc_word=next.tok.word;continue;}}else if(heredoc_word)continue;let tok2=toks[i+2];if(tok.ln && tok.ln>cur_line){have_comment=null;cur_line=tok.ln;}let ds=null;if(i>0 && toks[i-1].t=="ds"){ds=true;}if(!ds && tok.t=="word" && tok.word.match(/#/)&& !is_quote)have_comment=true;if(have_comment)continue;let ch=tok.c_op;let ch1=null;if(tok1)ch1=tok1.c_op;if(!ch){let prev=toks[i-1];let next=tok1;if(tok.word=="{"){if(ds)ch="{";else{if((prev && prev.word)||(next && next.word))continue;else ch="{"}}else if(tok.word==="}"){if((prev && prev.word)||(next && next.word))continue;}}if(ch=="{" || ch=="("){have_prev_c_op=false;if(!ds && i>0){let prev=find_not_blank_rev(toks,i-1);if(!prev || prev.t==="c_op")have_prev_c_op=true;if(!have_prev_c_op && prev && prev.t==="word"){prev=find_not_blank_rev(toks,prev.pos-1);if(!prev || prev.t=="c_op")have_prev_c_op=true;}if(!have_prev_c_op && prev && ch=="{"){if(prev &&(prev.t==="word" || prev.t==="quote")){}else have_prev_c_op=!!prev.c_op;}}else{if(ds && tok1 && tok1.word && tok1.word.match(/^[a-zA-Z]/)&& tok2 && tok2.word=="}"){toks[i]={'t':'sub','sub_t':'var','sub':tok1.word,ln:tok.ln};toks.splice(i-1,1);toks.splice(i,2);}else have_prev_c_op=true;}}if(ds && !ch){if(tok.word){let word=tok.word;let marr;if((marr=word.match(/^([*@#?0-9])(.*)$/))||(marr=word.match(/^([_a-zA-Z][_a-zA-Z0-9]*)(.*)$/))){toks[i]={'t':'sub','sub_t':'var','sub':marr[1],ln:tok.ln};if(marr[2])toks.splice(i+1,0,{'t':"word","word":marr[2],ln:cur_line});toks.splice(i-1,1);}}else if(toks[i].quote_t=="'"){tok['ds']=true;toks.splice(i-1,1);}}else if(((ch=="(" &&(have_prev_c_op))||(ch=="{" && have_prev_c_op)|| ch=="((")){if(is_quote){if(!ds)continue;}if(ds){toks.splice(i-1,1);i--;}let group=[];let type="cmd";let stack=[];let depth=0;if(ch=="(("){type="math";depth=1;if(from_math && !ds){ch="(";type="cmd";toks.splice(i+1,0,{"t":'c_op','c_op':"(",ln:cur_line});}}for(let j=i+1;toks[j];j++){let tokj=toks[j];let tokj1=toks[j+1];if(tokj.ln && tokj.ln>cur_line){have_comment=null;cur_line=tokj.ln;}if(tokj.t=="word" && tokj.word.match(/#/)){if(j>0 && toks[j-1].t=="ds"){}else if(j>0 && ds && tokj.word.match(/^#/)&& toks[j-1].word=="{"){}else have_comment=true;}if(have_comment)continue;if(depth==0){if(ch=="{" && tokj.word=="}" && !(toks[j-1].word ||(toks[j+1] && toks[j+1].word))){if(ds){toks[i]={'t':'sub','sub_t':"param",'sub':group,ln:cur_line};toks.splice(i+1,j-i);i=i+1;break;}else{let isgood=null;for(let k=j-1;k>0;k--){let tokk=toks[k];if(tokk==" "){continue;}let op=tokk[tokk.t];if(op==";" || op=="&" || op=="nl"){/*Need to make sure this is NOT empty braces*/ let grpstr=toks_to_string(group,true);if(grpstr.match(/^[ \n]*$/))return unexp_tok("}",tokj);let nexttok=find_not_blank(toks,j+1,false);if(nexttok && nexttok.tok.c_op){}else if(tokj1===" ")toks[j+1]={'t':'c_op','c_op':";",ln:cur_line};toks[i]={'t':'group','group_t':'shell','shell':true,'group':grpstr,ln:cur_line};toks.splice(i+1,j-i);i=i+1;isgood=true;break;}else{if(tokk.word && k===j-1)continue;sh_error("Could not find a valid statement terminator(';','&',or newline)for the shell group!");return;}}if(isgood)break;else return unexp_tok("}",tokj);}}else if(ch=="(" && tokj.c_op==")"){if(type=="cmd"){if(ds)toks[i]={'t':'sub','sub_t':"com",'sub':group,ln:cur_line};else{if(from_math){group.unshift("(");group.push(")");toks[i]={'t':'word','word':group,ln:cur_line};}else{let grpstr=toks_to_string(group,true);if(grpstr.match(/^[ \n]*$/)){return unexp_tok(")",tokj);}let nexttok=find_not_blank(toks,j+1,false);if(nexttok && nexttok.tok.c_op){}else if(tokj1===" ")toks[j+1]={'t':'c_op','c_op':";",ln:cur_line};toks[i]={'t':'group','group_t':"subshell",'subshell':true,'group':grpstr,ln:cur_line};}}toks.splice(i+1,j-i);i=i+1;break;}else if(type=="math" && tokj1 && tokj1.c_op==")"){make_math_group(group,ds,i,j,2);break;}}}if(tokj.c_op=="("){depth++;stack.push("(");}else if(tokj.word=="{"){depth++;stack.push("{");}else if(tokj.word=="}" && !(toks[j-1].word ||(toks[j+1] && toks[j+1].word))){depth--;if(stack.pop()!=="{"){return unexp_tok("}",tokj);}}else if(tokj.c_op==")"){depth--;if(depth==0 && type=="math" && tokj1 && tokj1.c_op==")"){make_math_group(group,ds,i,j,1);break;}else if(stack.pop()!=="(")return unexp_tok(")",tokj);}else if(tokj.c_op=="(("){depth=depth+2;stack.push("(","(");}if(tokj.t=="eof"){if(!global_com_name){return({'CONT':(main_com_str_arg || main_com_str)});}sh_error("end of file reached");return null;}group.push(tokj);}}else if(ch==="(" && !have_prev_c_op)return unexp_tok("(",tok);else if(ch===")"){if(is_quote)continue;return unexp_tok(")",tok);}}return toks;};this.groupify = groupify;
...
const redirectify=toks=>{if(!toks)return null;let ret=[];let ok_reds=["word","sub","quote"];for(let i=0;i<toks.length;i++){let is_heredoc=false;let esc_ch="";let tok0=toks[i];let tok1=toks[i+1];let tok2=toks[i+2];if(tok0.t=="r_op"){if(tok1 && tok1.t=="blank"){toks.splice(i+1,1);tok1=toks[i+1];tok2=toks[i+2];}if(tok0.r_op=="<<"){if(tok1.t=="esc"){esc_ch=tok1.esc;toks.splice(i+1,1);tok1=toks[i+1];tok2=toks[i+2];}if(tok1.t!="word"){sh_error("Need a simple word\x20(optionally escaped)\x20as a 'heredoc' delimiter");return null;}let word=	esc_ch+tok1.word;toks.splice(i+1,1);let j=i+1;let tokj=toks[j];while(tokj && !tokj.nl && !tokj.eof){j++;tokj=toks[j];}let got_delimiter=false;let here_doc=[];let on_nl=true;for(let k=j+1;k<toks.length;k++){let tok=toks[k];if(on_nl && tok.t=="word" && tok.word==word &&(toks[k+1].nl || toks[k+1].eof)){toks.splice(k,1);got_delimiter=true;break;}else{toks.splice(k,1);k--;if(tok.nl===true)here_doc.push("\n");else here_doc.push(tok);}if(tok.nl)on_nl=true;else on_nl=false;}if(!got_delimiter){/* * We can put a "heredoc" type of continuation here,so * the next quote strings('"\x60)are not treated special. They are * word tokens like any other */ return{'CONT':main_com_str,'TYPE':"heredoc"};}if(esc_ch){let arr=toks_to_string(here_doc).split("\n");if(!arr[0])arr.shift();if(!arr[arr.length-1])arr.pop();for(let i=0;i<arr.length;i+=2){if(i==1000000){throw new Error("If you really wanted 1000000 iterations here,then please by all means remove this infinite loop protector!");}arr[i]=arr[i].replace(/\x20$/,"");arr.splice(i+1,0,nl);}here_doc=arr;}tok0.here_doc=here_doc;ret.push(tok0);}else if(tok1&&ok_reds.includes(tok1.t)){let gottok;let redir=[];let j=i+2;for(j=i+1;j<toks.length;j++){let typ=toks[j].t;if(typ=="word"||typ=="sub"||typ=="quote")redir.push(toks[j]);else break;}tok0.redir_arg=redir;toks.splice(i+1,j-i-1);ret.push(tok0);}else{if(tok1.c_op)return unexp_tok(tok1,tok1);sh_error("Could not find a good redirection argument");return null;}}else ret.push(tok0);}return ret;}
...
const str_to_compound=(str)=>{let arr=str.split("\n");arr=escapes(arr);arr=quote_strings(arr);if(!arr)return;else if(arr.length==undefined)return arr;arr=tokify(arr);if(arr){let line=1;for(let i=0;i<arr.length;i++){if(arr[i].nl)line++;}}if(arr)arr.push({'t':'eof','eof':true});if(!arr)return;else if(arr.length==undefined)return arr;arr=groupify(arr);if(arr){if(arr.length==undefined)return arr;for(let i=0;i<arr.length;i++){if(arr[i].ds){arr[i].t="word";arr[i].ds=null;arr[i].word="$";}}}arr=strip_comments(arr);arr=redirectify(arr);if(arr && arr.CONT)return arr;arr=simples(arr);if(!arr)return;else if(arr.length==undefined)return arr;arr=pipelines(arr);if(arr){let ln_num=0;for(let i=0;i<arr.length;i++){if(arr[i].blank){arr.splice(i,1);i--;}else if(arr[i].nl){if(arr[i+1] && arr[i+1].nl){arr.splice(i,1);i--;}}}}arr=list_seq(arr,"pipeline","loglist","||","&&");arr=list_seq(arr,"loglist","list","&",";");if(arr){for(let i=1;i<arr.length;i++){if(arr[i].nl){arr.splice(i,1);i--;}}}return arr;}
...
const run_script=(main_com_str, final_cb, opts)=>{//«
...
const sys_read = arg => {
	...
	if (redir == undefined || redir.length == 0) {
...
const do_red = (com, redir_arr, cbarg, background_id) => {
				...
				if (have_and) {
					...
					ptw(fname, ret => {
						...
						if (termobj.file_objects[ukey]) {
							...
							if (ukey.match(/^\/serv\//)) {
								fs.get_serv_func(ukey, (funcret, errret) => {
									if (!funcret) {
										let mess = ukey + ":\x20";
										if (errret) mess += errret;
										else mess += "Cannot write to the file";
										com.error_message = mess;
										cbarg();
										return;
									}
									termobj.file_objects[ukey] = funcret;
									redir_arr[fdnum] = ukey;
									do_red(com, redir_arr, cbarg);
								});
							} else getfobj();
...
const assign_all_vars=(ass,_cb,if_tmp)=>{let i=-1;const doass=()=>{i++;if(i==ass.length){_cb();return;}let ass_str=ass[i].word;let all_toks=ass[i].all_toks;let match_arr=ass_str.match(/^(.*?)=(.*)$/);let ass_tok=match_arr[1];let ass_val=tilde_replace(match_arr[2]);let marr;if(all_toks && all_toks[0]){all_expansions(all_toks,ret=>{let setval;if(isarr(ret))setval=ret.join("\n");else{throw new Error("assign_all_vars():\x20No array from all expansions!");}set_var_str(ass_tok,setval,null,if_tmp);doass();});}else{let marr=ass_tok.match(/^([_a-zA-Z][_a-zA-Z0-9]*)(\[([_a-zA-Z0-9]+)\])?$/);let sub=marr[3];if(sub){if(sub.match(/^[0-9]/)){if(sub.match(/[^0-9]/)){sh_error(sub+":\x20invalid array subscript");_cb();return;}set_arr_val(marr[1],parseInt(sub),ass_val,if_tmp);}else set_obj_val(marr[1],marr[3],ass_val,if_tmp);}else set_var_str(ass_tok,ass_val,null,if_tmp);doass();}};doass();}
...
const set_var_str=(str,val,if_sys,if_tmp)=>{let use_env=var_env;if(if_tmp)use_env=tmp_env;if(!if_sys && constant_vars[str]){sys_abort=true;sh_error("attempted to assign to the constant:"+str);return "";}use_env[str]=val;}
...
const get_var_str=val=>{if(typeof(tmp_env[val])=="string"){return tmp_env[val];}else if(typeof(var_env[val])=="string"){return var_env[val];}return "";}
...
const tilde_replace=str=>{if(str=="~")return "/home/"+termobj.ENV.USER;else if(str.match(/^~\//))return str.replace(/^~/,"/home/"+termobj.ENV.USER);return str;}
const tilde_expansion=tok=>{if(!tok.t=="word")return;tok.word=tilde_replace(tok.word);}

const all_expansions=(main_arr_arg,exp_cb,opts)=>{if(!opts)opts={};let if_no_brace_exp=opts.NOBRACE;let if_array_sub=opts.ARRAYSUB;let if_heredoc=opts.HEREDOC;let if_herestr=opts.HERESTR;let ret=[];let last_num=null;let this_num=null;let curnum=0;let arr_arg;if(if_no_brace_exp)arr_arg=main_arr_arg;else arr_arg=brace_expansion_init(main_arr_arg);const next=if_no_set=>{if(!if_no_set)last_num=this_num;if(arr_arg.length)handle_expansion(arr_arg.shift());else{for(let i=0;i<ret.length;i++){if(ret[i]===""){ret.splice(i,1);i--;}}let yoom=0;for(let i=0;i<ret.length;i++){yoom++;if(yoom==10000){cerr("Infinite loop!");break;}if(isstr(ret[i])&& isstr(ret[i+1])){ret[i]+=ret[i+1];ret.splice(i+1,1);i--;}}if(if_heredoc || if_herestr)return(exp_cb(ret));fileglob(ret,globs=>{if(!globs[0])exp_cb(globs);else{let gotret=[];for(let i=0;i<globs.length;i++){if(globs[i].t=="blank" || globs[i]==" "){}else{let tok=globs[i];let str="";let tokj;if(typeof tok=="string")str=tok;else if(typeof tok=="object" && tok.t=="quote_string"){str=new String(tok.quote_string);str.is_quote=true;}let nexttok=globs[i+1];if(str &&(typeof nexttok=="string" ||(typeof nexttok=="object" && nexttok.t=="quote_string"))){let j=i+1;for(;tokj=globs[j];j++){if(tokj &&(tokj==" " || tokj.t=="blank")){if(str)gotret.push(str);str="";i=j;break;}else{if(typeof tokj=="string")str+=tokj;else if(typeof tokj=="object" && tokj.t=="quote_string"){str+=tokj.quote_string;str=new String(str);str.is_quote=true;}else{if(str)gotret.push(str);str="";gotret.push(tokj);i=j+1;break;}}}if(str){gotret.push(str);i=j+1;}}else{if(str)gotret.push(str);else gotret.push(tok);}}}exp_cb(gotret);}});}};const handle_expansion=tok=>{if(isstr(tok))tok={t:"word",word:tok};let type=tok.t;this_num=tok.tok_num;tok.tok_num=curnum;curnum++;if(type=="sub"){let subt=tok.sub_t;if(subt=="math"){get_math_val(tok.sub,math_ret=>{let last_ret=ret[ret.length-1];if(this_num && last_num &&((last_num+1)==this_num)&&(ret.length>1)&& typeof last_ret=="string")ret.push(ret.pop()+math_ret);else ret.push(math_ret+"");next();});}else if(subt=="com"){handle_com_tok(tok,com_ret=>{if(isarr(com_ret)){com_ret=chomp_eof(com_ret);if(if_heredoc)ret.push(...com_ret);else{if(com_ret.length){for(let rt of com_ret)ret.push(rt," ");ret.pop();}}}else if(isnum(com_ret))cwarn("com_ret returned:"+com_ret);else if(is_eof(com_ret))ret.push(EOF);else sh_error("all_expansions:got com_ret of type:"+(typeof com_ret));next();});}else if(subt=="var"){let gotval=var_subs(tok.sub,if_array_sub,tok.ln);if(gotval !=null){if(if_array_sub ||(typeof gotval=="object" && gotval.length !=undefined))ret.push({'t':"group_ret",'group_ret':gotval});else{if(isstr(gotval))gotval=gotval.split("\n").join(" ");else{cwarn("gotval from var_subs !==string");log(gotval);}ret.push(gotval);}}next();}else if(subt=="param"){param_subs(tok.sub,param_ret=>{if(param_ret !=null)ret.push(param_ret);next();});}}else if(if_heredoc){ret.push(tok);next();return;}if(type=="blank"){ret.push(tok);next();}else if(type=="esc"){ret.push(tok.esc);next();}else if(type=="word"){tilde_expansion(tok);if(!ret.length)ret.push(tok);else ret.push(tok['word']);next();}else if(type=="string" || type=="r_op"){ret.push(tok);next();}else if(type=="quote"){resolve_quote(tok,(func_ret,bq_ret)=>{if(bq_ret){if(isarr(bq_ret)){bq_ret=chomp_eof(bq_ret);if(bq_ret.length){for(let rt of bq_ret)ret.push(rt," ");ret.pop();}}else if(isnum(bq_ret))cwarn("bq_ret returned:"+bq_ret);else{cwarn("All_expansions():GOT bq_ret of type:"+(typeof bq_ret));log(bq_ret);}}else{if(!iseof(func_ret))ret.push(func_ret);}next();});}else if(tok.nl)next();else if(type=="group"){let groupt=tok.group_t;if(groupt=="math"){get_math_val(tok.group,math_ret=>{ret.push({'t':"group_ret",'group_ret':math_ret});next();});}else if(groupt=="subshell"){ret.push({'t':'subshell_group','subshell_group':tok.group,'redirects':tok.redirects,'assigns':tok.assigns});next();}else if(groupt=="shell"){ret.push({'t':'shell_group','shell_group':tok.group,'redirects':tok.redirects,'assigns':tok.assigns});next();}}else if(type !="sub"){cerr("All_expansions():WHAT TOK TYPE");log(tok);}};next();}
...
const all_sub_subs=(tok,cb)=>{let type=tok.sub_t;let sub=tok.sub;if(type=="param"){param_subs(sub,ret=>{cb({'RET':true,'VAL':ret});});}else if(type=="math"){math_subs(sub,ret=>{cb({'RET':true,'VAL':ret});});}else if(type=="com"){all_expansions([tok],ret=>{if(isarr(ret)&&ret.length==1&&iseof(ret[0]))cb({});else cb({'RET':true,'VAL':ret});});}else if(type=="var"){let gotval=var_subs(sub,null,tok.ln);if(gotval !=null)cb({'RET':true,'VAL':gotval});else cb({'RET':false});}else cerr("ALL SUBS TYPE????"+type);}
...
const math_subs=(math_arr_arg,math_cb)=>{const handle_math_expansion=math_tok=>{/*SHELLNOTES:6*/ let mtype=math_tok.t || typeof(math_tok);if(mtype=="word"){math_ret.push(math_tok.word);math_next();}else if(mtype=="string"){math_ret.push(math_tok);math_next();}else if(mtype=="sub"){let msubt=math_tok.sub_t;if(msubt=="param"){cerr("Need a math sub param!");}else if(msubt=="math"){math_subs(math_tok.sub,funcret=>{math_ret.push(funcret);math_next();});}else if(msubt=="com"){all_expansions(math_tok.sub,funcret=>{math_ret.push(funcret);math_next();});}else if(msubt=="var"){math_ret.push(get_var_str(math_tok.sub));math_next();}else cwarn("Handle math sub type:"+msubt);}else if(mtype=="paren"){/*We need to get the number value of the given expression,which may include any number of sub groups*/ cwarn("Got mtype=='paren'");jlog(math_tok);}else if(mtype=="quote"){if(math_tok.quote_t=="\x60"){cwarn("Got backquote in math_subs!");bq_abort(math_cb,4);return;}else{cerr("Handle the quote in math_subs");jlog(math_tok);}}else if(mtype=="mparen"){cwarn("Got mtype=='mparen'");jlog(math_tok);}else if(mtype=="r_op"){math_ret.push(math_tok.r_op);math_next();}else if(mtype=="c_op"){math_ret.push(math_tok.c_op);math_next();}else math_cb(null,"Unexpected token in the math expression,type="+mtype,true);};const math_next=()=>{if(math_arr_arg.length)handle_math_expansion(math_arr_arg.shift());else math_calc(math_ret,math_cb);};let math_ret=[];let arr=parser.groupify(math_arr_arg,{NAME:global_com_name,COM:main_com_str,MATH:true});if(!arr)return math_cb();let grouparr=renumber(arr,3);math_next();};
...
const var_subs=(str,if_array_sub,linenum)=>{if(str.match(/^[0-9]+$/)){let num=parseInt(str);if(global_args){if(num==0){if(global_com_name)return global_com_name;else{cerr("\n\n\nNO GLOBAL COM NAME?!?!?!?!?\n\n\n");return;}}else if(global_args[num-1])return global_args[num-1];else return "";}else{if(num==0)return sh_name();else return "";}}else if(str=="#"){if(global_args)return global_args.length+"";else return 0+"";}else if(str=="@" || str=="*"){if(global_args)return global_args.join(" ");else return "";}let use_env;if(!isnull(tmp_env[str]))use_env=tmp_env;else use_env=var_env;let marr;if(marr=str.match(/^([a-z_]+[a-z_0-9]*)\[(([0-9]+)([^0-9]+)?)\]/i)){if(marr[4])return sh_error(marr[2]+":\x20invalid array subscript",true);let got=use_env[marr[1]];if(isarr(got)){let val=got[parseInt(marr[3])];if(!val)return "";return val;}return "";}else if(marr=str.match(/^([a-z_]+)\[([a-z0-9_]+)\]/i)){let got=use_env[marr[1]];if(isobj(got)){let val=got[marr[2]];if(!val)return "";return val;}return "";}let gotval=use_env[str];if(isstr(gotval)|| isnum(gotval))return(gotval+"");else if(if_array_sub && isobj(gotval)){if(gotval.length !=undefined)return gotval;else return getkeys(gotval);}else if(isnull(gotval)){if(str=="LINENO"){if(global_com_name){if(!linenum)cwarn("var_subs:No linenum was passed in!");else return linenum+"";}else if(termobj.num_prompts)return(termobj.num_prompts)+"";}}return "";}
...
const expand_tok=(tok,cb,if_no_brace)=>{all_expansions([tok],ret=>{if(ret&&ret[0])cb(ret[0]);else cb();},{NOBRACE:if_no_brace});};
...
const handle_op=(obj, arg_vals_arg, next_arg_cb, in_background, if_need_or)=>{//«
...
const resolve_arg = (arg, cb, in_background, if_need_or) => {
	...
	const next_arg = (if_end, if_need_or) => {
		if (if_end) args = [];
		if (in_background && opcode == "loglist") {
			if (!check_job_id(in_background)) return;
		}
		resolve_arg(args.shift(), cb, in_background, if_need_or);
	};
	if (!arg) {
		cb(arg_vals);
		return;
...
if(opcode=="file"){resolve_arg(args.shift(),fileret=>{let retval=fileret.pop();execute_end(retval,null,3,in_background);});return;}
//else if(opcode=="word"){arg_vals_arg.push(args);if(next_arg_cb)next_arg_cb();return;}
else if(opcode=="c_op"){if(obj.c_op=="nl"||obj.c_op==";"){arg_vals_arg.push(args);if(next_arg_cb)next_arg_cb();}else{log(obj);cwarn("UNEXPECTED C_OP IN HANDLE_OP!!!!");sh_error("syntax error near unexpected token:"+tok_to_string(obj),true);if(next_arg_cb)next_arg_cb();}return;}
else if (opcode == "com") { /*SHELLNOTES:1*/
	let heredoc = null;
	let herestring = null;
	let com0 = obj.com[0];
	let use_redir;
	...
	const here_exp_to_arr = (arr, if_doc) => {
		...
		if (str) out.unshift(str);
		return out;
	};
	if (heredoc) {
		all_expansions(heredoc, ret => {
...
//	}
	...
	resolve_arg(args.shift(), coms => {
		...
		const execute_pipeline_command = (com, cb, pipenum) => { /*On the first call to this(which is the end of the pipeline chain,there is only a cb. All other calls only have a callback that "pipes in" to the previous call(which is later in the pipeline chain). call_com does not wait to start things up. It should return a function that actually calls the command and itself returns the pipein_cb. This function then goes into the com object,which opcode=="com" calls.*/
			if (in_background) {
		...
		const connect_pipes = () => { /*SHELLNOTES:4*/
...
//							if_done = true;
				...
				while (coms.length) {
					...
					com = coms.pop();
					cur_pipein_cb = execute_pipeline_command(com, null, coms.length);
				}
			}
		};
		const com_reds = err => {
			...
			if (com) { /*Filter out all non normal coms*/
				...
				if (typeof(retobj) == "number") {
					com_reds();
					return;
				} else if (retobj && retobj['NAME'] == "exec") {
					if (retobj['ARGC'] != 1) {
...
if(!args){if(typeof(obj)=="string"){arg_vals_arg.push(obj);if(next_arg_cb)next_arg_cb();}else{jlog(obj);sh_error("parse error:abort");ENV['?']=3;if(next_arg_cb)next_arg_cb();return;}}

const use_cb=funcret=>{if(funcret !=undefined)arg_vals_arg.push(funcret);if(next_arg_cb)next_arg_cb();};if(args){if(args.shift)resolve_arg(args.shift(),use_cb);else if(use_cb)use_cb();}
...
const call_com=(comarg, PIPES, cb, inback, pipenum)=>{//«
...
//Var«
const do_signin = async (args, if_up) =>{//«
	if (args.length) return cberr("Unexpected args!");

	let rv;
	rv = await fetch('/_status');
	if (rv.ok===true) return cberr("Already signed in!");

	let txt;
	let name=await getLineInput("Name:\x20");
    if (!name.match(/^[a-z][a-z0-9_]*$/i)) {
		cberr("Invalid username");
		return;
    } 
	let url;
	if (if_up) {
		url="/_signup?noweb=1";
		rv = await fetch(`/_checkavailable?name=${name}`);
		txt = await rv.text();
		if (rv.ok!==true) return cberr(txt);
	}
	else url = '/_signin';
	let pass1=await getLineInput("Password:\x20",true);
	if (if_up) {
		if (!pass1.match(/^[\x21-\x7e]+$/)) return cberr("Password must be printable ascii only");
		let pass2=await getLineInput("Confirm:\x20",true);
		if (pass1!==pass2) return cberr("Password mismatch");
	}
	let body=`${name}\n${pass1}`;
	if (if_up) {
		let email = (await getLineInput("Email (optional):\x20")).regstr();
		if (email && !(/^\S+@\S+[\.][0-9a-z]+$/.test(email))) return cberr("Invalid email");
		if (email) body += "\n"+email;
	}
	rv = await fetch(url,{method:"POST", body:body});
	txt = await rv.text();
	if (rv.ok!==true) return cberr(txt);
	if (if_up) cbok(txt);
	else cbok(`Signed in as: ${txt}`);
	let nm = await capi.refreshStats();
	if (_Desk && !if_up) _Desk.update_status(nm);
};
//»
//»
//Util«
...
const get_reader=()=>{return sys_read({ARG0:0,ARG1:pipe_arr,ARG2:get_redir(redir,0,pipenum),ARG3:PIPES,ARG4:cb,ARG6:true,ARG8:inback,PIPENUM:pipenum});}
...
const run_something = (namearg, strarg, argsarg, tmp_env_arg, opts, path) => {
	...
	if (path && strarg.match(/^#!js/)) {
		let usename = path.replace(/^\//, "").replace(/\//g, ".");
		if (NS.coms[usename]) {
...
const run_command = path => {
	...
	if (fullpath.match(/^\/serv\//)) {
		fs.get_serv_func(fullpath, (func, err) => {
			if (!func) return notfound();
			let reader = get_reader();
			if (!reader.is_device) return cberr("Services currently only read from devices");
			let is_done = false;
			kill_register(killcb => {
				is_done = true;
				cbok();
...
const lookup_file=async()=>{const notfound=()=>{sh_error(comword+":\x20command not found",null,get_redir(redir,2),127);wout(EOF);cb(ret_false(127));};const runit=async(path,obj)=>{if(obj.root.TYPE=="fs"){let rv=await fsapi.readHtml5File(path);run_something(comword,rv,arr,JSON.parse(JSON.stringify(tmp_env)),{},path);}else{cberr(path+":not(yet)running non Html5Fs type file");}};let path_arr=get_env_path();let found=false;for(let p of path_arr){let fullpath=normpath(p+"/"+comword);try{await fsapi.pathToNode(fullpath).then(o=>runit(fullpath,o));return;}catch(e){}}notfound();};
...
'signup':async(args)=>{do_signin(args,true);},
'signin':async(args)=>{do_signin(args);},
'signout': async (args) => {//«
	let rv = await fetch(`/_signout`);
	if (!rv) return cberr("Network error");
	let txt = await rv.text();
	if (!rv.ok) return cberr(txt);
	globals.stats.USERNAME = "";
	cbok(txt);
	if (_Desk) _Desk.update_status();
},//»
'status': async (args) => {
	let opts = failopts(args, {SHORT:{r:1}});
	if (!opts) return;
	if (opts.r) await capi.refreshStats();
	let uname = globals.stats.USERNAME;
	let email = globals.stats.EMAIL;
	if (!uname) return cbok("Not signed in");
	wout(`Signed in as: ${uname}`);
	wout(`Email: ${email}`);
	cbok();
//if ()
/*
...
'blobcat': (args) => {//«
	let which = args.shift();
	if (!which) return cberr();
	let blobs = ENV.BLOB_DROPS;
	if (!blobs) return cberr("No blobs in the env");
	let n = parseInt(which);
	if (!NUM(n) && n >= 0) return cberr("Invalid blob");
	let blob = blobs[n];
	if (!(blob instanceof Blob)) return cberr("No blob found");
	woutobj(blob);
	cbok();
},//»
...
'lib':args=>{let which=args.shift();if(!which)return cberr("No lib given!");let doit=async()=>{let keys=await NS.libs[which](Core,{});cbok(fmt(keys.sort().join(" ")));};if(NS.libs[which])doit();else Core.load_lib(which,ret=>{if(!ret)return cberr("No such module:\x20"+which);doit();});},
'help': async args => {
...
'cd': args => {
	...
	const cd_end = () => {
		if (!got_dir.match(/^\//)) got_dir = "/" + got_dir;
		cur_dir = got_dir;
		response({
			CD: got_dir,
		}, {
	...
	path_to_obj(regpath, ret => {
		...
		if (ret.APP != "sys.Explorer") return cberr(saypath + ":\x20Not a directory");
		got_dir = regpath;
		cd_end();
	});
},
...
'import': async args => {
	...
	const add_imports = async () => {
		...
		termobj.builtins = (getkeys(builtins).concat(getkeys(shell_lib))).sort();
		cbok("Successfully imported:\x20" + num + " functions from '" + which + "'");
		libs.push(which);
	};
	if (NS.libs[which]) {
		add_imports();
...
'wc':args=>{let _=shell_exports;let curfile=null;let words=null,chars;let numlines;_.read_file_args_or_stdin(args,(ret,filearg,errarg)=>{if(filearg){curfile=filearg;return;}else if(errarg){_.werr(errarg);return;}if(isobj(ret)&&ret.EOF){if(words)_.wout(numlines+"  "+words+"  "+chars);return _.cbok();}if(isarr(ret)){words=0,chars=0;for(let i=0;i<ret.length;i++){let warr=ret[i].split(/\x20+/);words+=warr.length;for(let j=0;j<warr.length;j++)chars+=warr[j].length;}let strout=ret.length+"  "+words+"  "+chars;if(curfile)strout+="  "+curfile;words=null;_.wout(strout);}else if(isstr(ret)){if(words===null){words=0;chars=0;numlines=0;}numlines++;let warr=ret.split(/\x20+/);words+=warr.length;for(let i=0;i<warr.length;i++)chars+=warr[i].length;}else if(!ret)_.cberr("no input");else{cwarn("wc:what input?");log(ret);}},{SENDEOF:true});},
'pipe': args => {
	let opts = failopts(args, {
		LONG: {
			append: 3
	...
	const lineout = line => {
		for (let i = 0; i < line.length; i++) {
			...
			if (args.length == 1 && line.length > 1) {
				set_var_str(args[0], line.splice(i).join(" "));
				break;
			}
			set_var_str(args[0], line[i]);
			args.shift();
		}
...
'devcat':args=>{if(!failopts(args))return;com_cat(args,{OBJOK:1});},
...
'echo':args=>{let sws=failopts(args,{SHORT:{r:1,n:1,o:1}});if(!sws)return;if(sws.o){if(args.length==1)wout(args[0]);else woutarr(args);wout(EOF);cbok();}else{let str;if(!args.length)str="";else str=args.join(" ");if(sws.r)str=str.split("").reverse().join("");wout(str,{NONL:sws.n});wout(EOF);cbok();}},
'clear':()=>{termobj.clear();},
...
if (!arr && comarg.length) return cb(comarg[0]);
	...
	if (comword) {//«
		...
		else if (!isquote && shell_lib[comword]) shell_lib[comword]._call(shell_exports, arr);
		else if (!isquote && is_root && sys_builtins[comword]) sys_builtins[comword](arr);
		else {//«
			if (comword.match(/\//)) run_command(comword);
			else lookup_file();
		}//»
	}//»
