/*New Gzip/DEFLATE: https://wicg.github.io/compression/
...
*/
/*XXX MAJOR LESSON WHEN CALLING STDIN XXX«

Calling read_stdin in shell.js is the way that pipein_cb is set and returned from call_com, which
is the way that pipelines are constructed inside of handle_op, where opcode=="pipeline". This is all
done "inline"/synchronously, with the lesson being that calling read_stdin (or _.stdin here) must
be done immediately, and not waited for, eg, calling path_to_obj to see if a file exists.
This lesson was learned when the 'tee' command tried to perform that exact kind of argument
validation before calling read_stdin. The read_stdin call will just hang and (almost) never get 
anything.
...
await Core.api.loadMod("util.ZipJS");
await Core.api.loadMod("util.ZipJSDeflate");
if (!globals.zip)new NS.mods["util.ZipJS"](Core);
let zip = globals.zip;
if (!zip.Deflater)new NS.mods["util.ZipJSDeflate"](Core);
...
await Core.api.loadMod("util.ZipJS");
await Core.api.loadMod("util.ZipJSInflate");
if (!globals.zip)new NS.mods["util.ZipJS"](Core);
let zip = globals.zip;
if (!zip.Inflater)new NS.mods["util.ZipJSInflate"](Core);
if (fname) {
	atbc(fname,ret=>{//«
...
let start=str=>{
	Core.load_mod("util.vim",async rv=>{
		...
		let vim = new NS.mods["util.vim"](Core, shell_exports);
		let useext;
		if (sws.b) {
...
let begin=()=>{//«
	const dopath=()=>{//«
		...
		ptw(fullpath, async fobj=>{
			const checkok=()=>{
				if (rtype=="remote") {
					if (!fs.check_user_perm(fobj,null,dsk)) return cberr("Permission denied: " + arg);
					return true;
				}
				if (rtype=="local"){
			...
			if (!fobj) {
				...
				fileobj=fobj;
//				if (!(!fobj.APP || fobj.APP.match(/text/i))) return cberr("Cannot edit file type: " + fobj.APP);
				if (fobj.fullpath) fullpath = fobj.fullpath;
				rootobj = fobj.root;
				rtype = rootobj.TYPE;
				parobj = fobj.par;
				if (!checkok())  return;
				if (rtype=="remote") fs.get_rem_file(fullpath,start,{NOCACHE: true})
				else {
					let modtime = null;
					...
					fs.read_file(fullpath, async(rv)=>{
						...
						if (isarr(rv)&&isstr(rv[0])) {

if (modtime && vimstore){
	let rv2 = await vimstore.get(sysfullpath);
	if (rv2){
		let obj = await capi.decompress(rv2,{toObj:true});
		let diff = Math.floor((obj.time-modtime)/1000);
		if (diff > 1){
			werr(fmt(`The automatically stored state is newer than the saved version by: ${diff}s!`));
			termobj.set_prompt("[U]se or [D]elete it? ",{FROMCOM:true});
			refresh();
			termobj.getch(async ch=>{
				if (ch=="u"||ch=="U") {
					werr("USING the stored version...");
					setTimeout(()=>{start(obj);},666);
					return 
				}
				if (ch=="d"||ch=="D") {
					werr(fmt("DELETING the stored version and REVERTING to saved...)"));
					await vimstore.delete(sysfullpath);
					setTimeout(()=>{start(rv.join("\n"));},666);
					return;
				}
				werr(fmt("NOT DELETING the stored version, and REVERTING to saved..."));
				setTimeout(()=>{start(rv.join("\n"));},666);
			});
			return;
		}
	...
	'less':function() {//«
		...
		let doless=val=>{
			Core.load_mod("util.pager",rv=>{
				if (!rv) return cberr("No util.pager module!");
				let less = new NS.mods["util.pager"](Core, shell_exports);
				didless = true;
				addlines_cb = less.init(val, name, ()=>{
					addlines_cb = null;
					cbok();
				});
			});
...
log(obj);
					...
					ptw(parpath, parobj=>{
						...
						fs.touch_fs_file(fullpath, ret=>{
							...
							if (Core.Desk) {
								if (dsk) dsk.Desk.make_icon_if_new(fullpath);
								else Core.Desk.make_icon_if_new(fullpath);
							}
							dotouch();
						},null, is_root, dsk)
					});
...
const coms_help={//«
vim:`vi/vim is the canonical text editor for POSIX-based systems.

It is different from other editors because it is mode based, and the default mode (command) does not accept textual input.

To start typing text, press 'i', which stands for "insert".

Then, press Escape to get back into command mode.

Alt+s is the system-wide way to save files in ${SYSACRONYM}.
`
};//»

if (!com) return new Promise((y,n)=>{y(Object.keys(coms));});
if (!args) return coms_help[com];
if (!coms[com]) return cberr("No com: " + com + " in fs!");
...
//					let man = new NS.mods["sys.pager"](Core, termobj);
					let man = new NS.mods["util.pager"](Core, shell_exports);
