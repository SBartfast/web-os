...
const{api:capi,KC,kc,log,cwarn,cerr}=Core;
const{util}=globals;
...
const start=async()=>{//«
	...
	const load_mods=async()=>{
		...
		if (typeof rv === "string") Core.do_update("mods.sys.shell", rv);
		shell = new mods["sys.shell"](Core, termobj);
		rv = await capi.loadMod("util.pager");
		if (!rv) resperr("Could not load the system pager!");
		if (typeof rv === "string") Core.do_update('mods.util.pager', rv);
	...
	const init_or_load=async()=>{
		...
		else if (mods["sys.shell"]&&mods["util.pager"]) {
			shell = new mods["sys.shell"](Core, termobj);
			await execute(load_com, treat_as_init);
			cursorinit();
		}
