...
FS999
fs.getfile => get_fs_by_path => api.readHtml5File
...
Step 4) Implement new interfaces to vim, less, man, etc.


»*/

//Development LIB/MOD deleting«
const DEL_LIBS=[
	"math.trading",
//	"admin"
//	"wasm"
...
this.onkill = (if_dev_reload)=>{//«
	let keys = termobj.file_objects.keys;
	for (let k of keys) {
...
//		let colobj = line_colors[curnum]||arr.tcolor;
		let colobj = line_colors[curnum];
//if (colobj&&colobj.keys.length)log("COLOBJ",colobj);
...
const get_homedir=()=>{
	if (dsk) return dsk.home_path;
	let name = Core.get_username();
	if (!name) return "/";
	if (fsapi.getRoot().KIDS.home.KIDS[name]) return"/home/"+name;
	return "/";
};
...
const end_app_mode=(if_no_cb, last_str)=>{//«
	term_mode = "shell";
	buffer = buffer_hold;
...
const do_ctrl_D=()=>{//«
	if (term_mode != "shell") {
		...
		if ((cur_prompt_line  ==  cy()) && (x  == prompt_len)) end_app_mode(null, str);
...
const get_command_arr=async (dir, pattern, cb)=>{//«
	...
	if (match_arr.length) return cb(match_arr);

	let dokids = kids=>{
		if (!kids) return;
		let keys = kids.keys;
		for (let k of keys){
			if (!kids[k].APP && re.test(k)){
...
const execute=(str, if_init)=>{//«
	...
	return shell.execute(str, cur_dir, {init:if_init, root:root_state});
};
...
//log(user);
	...
	if (ENV.USER) {
		...
		else if ((new RegExp("^/home/"+ENV.USER+"/")).test(cur_prompt)) cur_prompt = cur_prompt.replace(/^\/home\/[^\/]+\//,"~/");
	}
...
const get_dir_contents=async(dir, pattern, cb)=>{//«
	...
	if (!(ret&&ret.APP=="sys.Explorer")) return cb([]);
...
const handle_tab=()=>{//«
	...
	const docontents=async()=>{//«
		if (contents.length == 1) {//«
			let chars = contents[0][0].replace(/[ #~{}<>|&!,()]/g, "\\$&").split("");
...
//					fs.path_to_obj(link, obj=>{}, root_state, false, dsk);
					if (obj&&obj.APP=="sys.Explorer") {
						if (await_next_tab) {
							handle_letter_press("/");
							await_next_tab = null;
						}
...
Where POP3.html is some large html files, and htparse dumps the formatted text to stdout, which is
piped through less...
$ cat POP3.html | htparse | less

After pressing 'q' to exit less, the prompt is NOT returned, and pressing the UP key runs into this
situation...
...
const start=async()=>{//«
	...
	const load_mods=async()=>{
		let ret2 = await capi.loadMod("sys.shell");
		if (!ret2){
			main.pad=10;
			main.bgcol="#000";
			main.tcol="#aaa";
		...
		if (typeof ret2 === "string") Core.do_update("mods.sys.shell", ret2);
		shell = new mods["sys.shell"](Core, termobj);
		let ret3 = await capi.loadMod("util.pager");
		if (!ret3) resperr("Could not load the system pager!");
		if (typeof ret3 === "string") Core.do_update('mods.util.pager', ret3);
		await execute(load_com, true);
		cursorinit();
	}
	const init_or_load=async()=>{
		if (mods["sys.shell"]&&mods["util.pager"]) {
			shell = new mods["sys.shell"](Core, termobj);
			await execute(load_com, treat_as_init);
			cursorinit();
		}
		else load_mods();
	}
	sleeping = true;
	let load_com;
	let ret = await fsapi.readHtml5File("/home/"+Core.get_username()+"/.bashrc");
	cur_dir = get_homedir();
	if (ret) load_com = ret;
	else load_com = ":";
	init_or_load();
...
else if (arg.HISTORY) buffer = arg.HISTORY;
